...
try {
  ...
  await evaluate('window.savedBar=document.querySelector("#allocation span"); window.oldWidth=savedBar.getBoundingClientRect().width; const x=document.querySelector("#weights input[type=number]"); x.value=80; x.dispatchEvent(new Event("input"));');
  assert.equal(await evaluate('savedBar === document.querySelector("#allocation span")'),true);
  assert.equal(await evaluate('parseFloat(getComputedStyle(savedBar).transitionDuration)>0'),true);
  await evaluate('new Promise(r=>setTimeout(r,350))');
  assert.equal(await evaluate('savedBar.getBoundingClientRect().width > oldWidth'),true);
  await call('Emulation.setEmulatedMedia',{features:[{name:'prefers-reduced-motion',value:'reduce'}]});
  await evaluate('document.querySelectorAll("#mode-options button")[1].click()');
  assert.equal(await evaluate('document.getAnimations().length'),0);
  assert.equal(await evaluate('getComputedStyle(document.querySelector("#allocation span")).transitionDuration'),'0s');
